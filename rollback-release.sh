#!/bin/bash
# Запускается как failCmd плагина @semantic-release/exec, если semantic-release
# упал НА ЛЮБОМ шаге ПОСЛЕ того, как @semantic-release/git уже закоммитил и
# запушил "chore(release): X.Y.Z" в main. Без этого main застревает с бампнутой
# версией/CHANGELOG, а тега и npm publish при этом может не быть (см. историю
# с 6.0.18/6.0.19).
#
