...
export function CW_MixedWidthAndAuto() {
    const { cols, onColumnResize } = useResizableCols(
        withAdaptedGrow([
            { title: "ID (w:80)", id: "id", width: 80 },
            { title: "Title (auto, max:3000)", id: "title", maxWidth: 3000 },
            { title: "Priority (w:150)", id: "priority", width: 150 },
...
// 12. 30 колонок с minWidth/maxWidth, без grow (totalWidth > clientWidth → grow-if не срабатывает)
function make30Columns(): GridColumn[] {
    const defs: Array<{ title: string; id: string; width: number; minWidth: number; maxWidth: number }> = [];
    for (let i = 0; i < 30; i++) {
        const base = 60 + (i % 5) * 20;
        defs.push({
            title: `Col${i} (${base}px, min:${base - 20}, max:${base + 40})`,
            id: `col${i}`,
            width: base,
            minWidth: base - 20,
            maxWidth: base + 40,
...
function ColumnInfoPanel({ cols }: { cols: GridColumn[] }) {
    return (
        <div
            style={{
                fontFamily: "monospace",
            ...
            <table style={{ borderCollapse: "collapse", width: "100%" }}>
                <thead>
                    <tr style={{ borderBottom: "2px solid #999", textAlign: "left" }}>
                        ...
                        <th style={{ padding: "2px 6px" }}>width</th>
                        <th style={{ padding: "2px 6px" }}>min</th>
                        <th style={{ padding: "2px 6px" }}>max</th>
                        <th style={{ padding: "2px 6px" }}>grow</th>
                    </tr>
                </thead>
                <tbody>
                    {cols.map((c, i) => (
                        <tr key={c.id ?? i} style={{ borderBottom: "1px solid #eee" }}>
                            ...
                            <td style={{ padding: "2px 6px", fontWeight: "bold" }}>{c.width ?? "auto"}</td>
                            <td style={{ padding: "2px 6px" }}>{c.minWidth ?? "-"}</td>
                            <td style={{ padding: "2px 6px" }}>{c.maxWidth ?? "-"}</td>
                            <td style={{ padding: "2px 6px" }}>{c.grow ?? "-"}</td>
                        </tr>
                    ))}
                </tbody>
            </table>
...
export function CW_30ColumnsMinMax() {
    const { cols, onColumnResize } = useResizableCols(
        withAdaptedGrow(make30Columns())
    );
    return (
        <div style={{ display: "flex", gap: 16, width: 1600, height: 600 }}>
...
CW_30ColumnsMinMax.decorators = [];

// 13. Одна колонка с maxWidth:3000 и grow:1
export function CW_SingleColumnMax3000() {
    const { cols, onColumnResize } = useResizableCols(
        withAdaptedGrow([
            { title: "Title (max:3000, maxAuto:3000)", id: "title", maxWidth: 3000, maxAutoWidth: 3000 },
        ])
    );
    return (
        <WidthStoryShell
...
CW_SingleColumnMax3000.decorators = [];

// 14. Сайдбар: изменение ширины контейнера с grow-колонками
...
export function CW_SidebarResize() {
    ...
    return (
        <div style={{ display: "flex", width: 1600, height: 600 }}>
            <div
                ref={containerRef}
                style={{
