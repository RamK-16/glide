...
- `render/data-grid-render.ts` — `clipHeaderDamage`: (а) клип слитой колонки на всю высоту,
  (б) **allSpanned-скип в групп-цикле** (см. инвариант ниже).

### ⚠️ Инвариант damage/clip слитой шапки (дорого далось — не сломать)

**Правило:** `clipHeaderDamage` (групп-цикл по `walkGroups`) и `drawGroupLevel` ОБЯЗАНЫ
одинаково пропускать `allSpanned`-спаны (групп-спан, где ВСЕ колонки — `spanGroupHeader`).
Обе точки помечены перекрёстными комментариями.

**Почему (баг, который чинили):** сценарий `SGH_Basic`, наводим на группу «Коэффициент
результативности». Это задевает групп-ряд СОСЕДА — Периода: `damage=[[2,-2]]`. Пустой
групп-спан `[1,2]` (Краткое+Период, обе слитые) в норме НЕ рисуется (`drawGroupLevel` его
скипает как `allSpanned` — групп-ряд слитой колонки восстанавливает её собственная
перерисовка на всю высоту). Но `clipHeaderDamage` этот спан НЕ скипал → ставил клип на его
групп-полосу (x Краткого+Периода) → очистка фона `fillRect(0,0,width,total)` внутри клипа
**стирала верхнюю полосу Краткого**. Восстановить некому: `damage` самого Краткого нет
(`drawGridHeaders` его пропускает), а `drawGroups` спан пропускает как `allSpanned`.
Итог: серая полоса поверх текста Краткого + исчезнувшая вертикальная линия.

**Как ловили (на будущее):** визуально серое сливалось с hover-серым (`bgHeaderHovered`
`#EFEFF1` ≈ `bgHeader` `#F7F7F8`). Решающий приём — **пиксельный зонд `getImageData`
с активной ячейкой в колонке**: фокус-цвет `bgHeaderHasFocus` (тёмный) развёл верх/низ и
показал `top=247 (обычный фон) / bot=233 (фокус)` — т.е. верх затирается ОТДЕЛЬНО и ПОСЛЕ
заливки колонки. Лог `spanFull=true, y=0, drawH=70` подтвердил, что сама колонка рисуется
слитно на всю высоту → значит виновата очистка внутри клипа, а не рендер колонки.

**Общность:** фикс структурный — покрывает ЛЮБОЙ случай, когда групп-ряд слитой колонки
попадает в клип из-за задетого соседа (любая соседняя группа/колонка, оба направления),
не только этот сценарий. Для будущего group-rowspan (§3-4) тот же инвариант распространить
на merged-регионы групп.
